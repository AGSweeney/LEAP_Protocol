...
EipStatus CipStackInit(const EipUint16 unique_connection_id) {
  ...
  OPENER_ASSERT(kEipStatusOk == eip_status);
  eip_status = CipLldpDataTableInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
#endif
...
void ShutdownCipStack(void) {
  ...
  ShutdownAssemblies();
  /* agsweeney@gmail.com: release dynamically allocated identity product name */
  ...
  for(new_instances = 0; new_instances < number_of_instances; new_instances++) {
    ...
    *next_instance = current_instance; /* link the previous pointer to this new node */
    ...
    cip_class->number_of_instances =
      (EipUint16)(cip_class->number_of_instances + 1U); /* update the total number of instances recorded by the class */
...
CipClass *CreateCipClass(const CipUdint class_code,
  ...
  cip_class->number_of_attributes =
    (EipUint16)number_of_instance_attributes; /* the class remembers the number of instances of that class */
  cip_class->highest_attribute_number =
    (EipUint16)highest_instance_attribute_number; /* indicate which attributes are included in instance getAttributeAll */
  cip_class->number_of_services =
    (EipUint16)number_of_instance_services; /* the class manages the behavior of the instances */
  ...
  meta_class->number_of_attributes =
    (EipUint16)(number_of_class_attributes + 7); /* the metaclass remembers how many class attributes exist*/
  meta_class->highest_attribute_number =
    (EipUint16)highest_class_attribute_number; /* indicate which attributes are included in class getAttributeAll*/
  meta_class->number_of_services =
    (EipUint16)number_of_class_services; /* the metaclass manages the behavior of the class itself */
...
void InsertAttribute(CipInstance *const instance,
  ...
  for(int i = 0; i < instance->cip_class->number_of_attributes; i++) {
    if(attribute->data == NULL) { /* found non set attribute */
      ...
      const uint8_t attribute_bit = (uint8_t)(1U << (attribute_number % 8U));
      if(0 != (cip_flags & kGetableSingle)) {
        cip_class->get_single_bit_mask[index] =
          (uint8_t)(cip_class->get_single_bit_mask[index] | attribute_bit);
      }
      if(0 != (cip_flags & (kGetableAll | kGetableAllDummy))) {
        cip_class->get_all_bit_mask[index] =
          (uint8_t)(cip_class->get_all_bit_mask[index] | attribute_bit);
      }
      if(0 != (cip_flags & kSetable)) {
        cip_class->set_bit_mask[index] =
          (uint8_t)(cip_class->set_bit_mask[index] | attribute_bit);
      }
...
void EncodeCipShortString(const void *const data,
  ...
  size_t bytes_to_copy = short_string->length;
  /* agsweeney@gmail.com: clamp short-string copy to remaining ENIP message space. */
  const size_t remaining_space = sizeof(outgoing_message->message_buffer) -
                                 outgoing_message->used_message_length;
  if(bytes_to_copy > remaining_space) {
    OPENER_TRACE_ERR("EncodeCipShortString: truncating data from %u to %u bytes\n",
  ...
  memcpy(outgoing_message->current_message_position,
         short_string->string,
         bytes_to_copy);
  outgoing_message->current_message_position += bytes_to_copy;
  outgoing_message->used_message_length += bytes_to_copy;
}
...
void EncodeCipString(const void *const data,
  ...
  if(0 != string->length) {
    size_t bytes_to_copy = string->length;
    /* agsweeney@gmail.com: clamp string copy to remaining ENIP message space. */
    const size_t remaining_space = sizeof(outgoing_message->message_buffer) -
                                   outgoing_message->used_message_length;
    if(bytes_to_copy > remaining_space) {
      OPENER_TRACE_ERR("EncodeCipString: truncating data from %u to %u bytes\n",
                       (unsigned)bytes_to_copy,
                       (unsigned)remaining_space);
      bytes_to_copy = remaining_space;
    }
...
int DecodeCipDword(CipDword *const data,
  ...
  *data = GetDwordFromMessage(&message_router_request->data);
  message_router_response->general_status = kCipErrorSuccess;
  return 4;
}
...
int DecodeCipLint(CipLint *const data,
  ...
  *data = (CipLint)GetLintFromMessage(&message_router_request->data);
  message_router_response->general_status = kCipErrorSuccess;
  return 8;
}
...
int DecodeCipReal(CipReal *const data,
  ...
  EipUint32 raw_value = GetDwordFromMessage(&message_router_request->data);
  memcpy(data, &raw_value, sizeof(raw_value));
  message_router_response->general_status = kCipErrorSuccess;
  return 4;
}
...
int DecodeCipLreal(CipLreal *const data,
  ...
  EipUint64 raw_value = GetLintFromMessage(&message_router_request->data);
  memcpy(data, &raw_value, sizeof(raw_value));
  message_router_response->general_status = kCipErrorSuccess;
  return 8;
}
...
EipStatus GetAttributeAll(CipInstance *RESTRICT const instance,
                          CipMessageRouterRequest *const message_router_request,
                          CipMessageRouterResponse *const message_router_response,
                          const struct sockaddr *originator_address,
                          const CipSessionHandle encapsulation_session) {
  ...
  if(0 == instance->cip_class->number_of_attributes) {
    ...
    for(size_t j = 0; j < instance->cip_class->number_of_attributes; j++) {
      ...
      if( (instance->cip_class->get_all_bit_mask[CalculateIndex(attribute_number)
        ...
        /* Keep GetAttributeAll behavior consistent with GetAttributeSingle for
         * attributes that require dynamic refresh (e.g. Ethernet Link counters). */
        if( (attribute->attribute_flags & kPreGetFunc) &&
            NULL != instance->cip_class->PreGetCallback ) {
          instance->cip_class->PreGetCallback(instance,
                                              attribute,
...
EipStatus GetAttributeList(CipInstance *RESTRICT const instance,
                           CipMessageRouterRequest *const message_router_request,
                           CipMessageRouterResponse *const message_router_response,
                           const struct sockaddr *originator_address,
                           const CipSessionHandle encapsulation_session) {
  ...
  if(message_router_request->request_data_size < sizeof(CipUint)) {
    message_router_response->general_status = kCipErrorNotEnoughData;
    return kEipStatusOkSend;
  }

  const EipUint8 *request_data_cursor = message_router_request->data;
  size_t request_data_remaining = message_router_request->request_data_size;
  CipUint attribute_count_request = GetUintFromMessage(&request_data_cursor);
  request_data_remaining -= sizeof(CipUint);

  if((size_t)attribute_count_request > (request_data_remaining / sizeof(CipUint))) {
    message_router_response->general_status = kCipErrorNotEnoughData;
    return kEipStatusOkSend;
  }
  ...
  if(0 != attribute_count_request) {
    ...
    for(size_t j = 0; j < attribute_count_request; j++) {
      if(request_data_remaining < sizeof(CipUint)) {
        message_router_response->general_status = kCipErrorNotEnoughData;
        return kEipStatusOkSend;
      }

      attribute_number = GetUintFromMessage(&request_data_cursor);
      request_data_remaining -= sizeof(CipUint);
...
EipStatus SetAttributeList(CipInstance *RESTRICT const instance,
                           CipMessageRouterRequest *const message_router_request,
                           CipMessageRouterResponse *const message_router_response,
                           const struct sockaddr *originator_address,
                           const CipSessionHandle encapsulation_session) {
...
void EncodeEPath(const void *const data,
  ...
  const size_t start_length = message->used_message_length;
...
EipStatus DecodePaddedEPath(CipEpath *epath,
  ...
  if(NULL == message || NULL == *message || available_bytes < 1U) {
  ...
  const size_t declared_path_bytes = (size_t)epath->path_size * sizeof(CipWord);
  const size_t total_declared_bytes = declared_path_bytes + 1U; /* includes path_size octet */
  if(total_declared_bytes > available_bytes) {
    return kEipStatusError;
  }

  const EipUint8 *const path_end = message_runner + total_declared_bytes;
  message_runner++;
  ...
  while(number_of_decoded_elements < epath->path_size) {
    ...
    switch(*message_runner) {
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_CLASS_ID +
        ...
        if((size_t)(path_end - message_runner) < 2U) {
          return kEipStatusError;
        }
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_CLASS_ID +
        ...
        if((size_t)(path_end - message_runner) < 4U) {
          return kEipStatusError;
        }
        message_runner += 2;
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_CLASS_ID +
        ...
        if((size_t)(path_end - message_runner) < 6U) {
          return kEipStatusError;
        }
        message_runner += 2;
        {
          const CipUdint class_id = GetUdintFromMessage( &(message_runner) );
          /* agsweeney@gmail.com: reject 32-bit logical values that cannot fit legacy request path fields */
          if(class_id > UINT16_MAX) {
            return kEipStatusError;
          }
          epath->class_id = (EipUint16) class_id;
        }
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_INSTANCE_ID +
        ...
        message_runner += 2;
        {
          const CipUdint instance_id = GetUdintFromMessage( &(message_runner) );
          if(instance_id > UINT16_MAX) {
            return kEipStatusError;
          }
          epath->instance_number = (CipInstanceNum) instance_id;
        }
        number_of_decoded_elements += 2;
        break;
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_ATTRIBUTE_ID +
        ...
        message_runner += 2;
        {
          const CipUdint attribute_id = GetUdintFromMessage( &(message_runner) );
          if(attribute_id > UINT16_MAX) {
            return kEipStatusError;
          }
          epath->attribute_number = (EipUint16) attribute_id;
        }
        number_of_decoded_elements += 2;
        break;
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_MEMBER_ID +
        ...
        number_of_decoded_elements += 2;
        break;
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_CONNECTION_POINT +
        LOGICAL_SEGMENT_FORMAT_EIGHT_BIT:
        if((size_t)(path_end - message_runner) < 2U) {
          return kEipStatusError;
        }
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_CONNECTION_POINT +
        ...
        number_of_decoded_elements += 2;
        break;
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_SERVICE_ID +
        LOGICAL_SEGMENT_FORMAT_EIGHT_BIT:
        if((size_t)(path_end - message_runner) < 2U) {
          return kEipStatusError;
        }
      ...
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_EXTENDED_LOGICAL +
        ...
        /* agsweeney@gmail.com: consume segment+type+value and 16-bit path padding byte */
        message_runner += 4;
        number_of_decoded_elements++;
        break;
      case SEGMENT_TYPE_LOGICAL_SEGMENT + LOGICAL_SEGMENT_TYPE_EXTENDED_LOGICAL +
  ...
  *bytes_consumed = total_declared_bytes;
  return kEipStatusOk;
}
