...
/* Optional platform hook: clear source counters after successful GetAndClear.
 * Default is no-op; platform ports may provide a strong definition. */
void OpenerEthernetLinkPlatformClearCounters(CipInstanceNum instance_number,
                                             EipUint16 attribute_number)
  __attribute__((weak));
void OpenerEthernetLinkPlatformClearCounters(CipInstanceNum instance_number,
                                             EipUint16 attribute_number) {
  (void)instance_number;
  (void)attribute_number;
}
...
/* Dummy variable to provide fill data for the GetAttributeAll service. */
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
#else
...
EipStatus CipEthernetLinkInit(void) {
  ...
  for (size_t idx = 0; idx < OPENER_ETHLINK_INSTANCE_CNT; ++idx) {
    ...
    g_ethernet_link[idx].admin_state = kEthLinkAdminStateEnabled;
    g_ethernet_link[idx].interface_state = kEthLinkIfStateUnknown;
#if defined(OPENER_ETHLINK_LABEL_ENABLE) && 0 != OPENER_ETHLINK_LABEL_ENABLE
  ...
  if (ethernet_link_class != NULL) {
    ...
    for (CipInstanceNum idx = 0; idx < OPENER_ETHLINK_INSTANCE_CNT; ++idx) {
      ...
      InsertAttribute(ethernet_link_instance,
                      ...
                      EncodeCipUsint,
                      NULL,
                      &g_ethernet_link[idx].admin_state,
                      kGetableSingleAndAll);
      InsertAttribute(ethernet_link_instance,
                      10,
                      kCipShortString,
...
void CipEthernetLinkUpdateLinkStatus(CipUdint speed_mbps,
                                     CipBool full_duplex,
                                     CipBool link_up) {
  uint16_t selector = kEthLinkSpeedDpx_100_FD;
  ...
  if(speed_mbps >= 1000U) {
    selector = full_duplex ? kEthLinkSpeedDpx_1000_FD : kEthLinkSpeedDpx_1000_HD;
  } else if(speed_mbps >= 100U) {
    selector = full_duplex ? kEthLinkSpeedDpx_100_FD : kEthLinkSpeedDpx_100_HD;
  } else {
    selector = full_duplex ? kEthLinkSpeedDpx_10_FD : kEthLinkSpeedDpx_10_HD;
  }

  if(OPENER_ETHLINK_INSTANCE_CNT > 0U) {
    g_ethernet_link[0].interface_speed = speed_mbps;
    g_ethernet_link[0].interface_flags = link_up ? (CipUsint)0x0FU : (CipUsint)0x00U;
    g_ethernet_link[0].interface_caps.speed_duplex_selector = selector;
    if(kEthLinkAdminStateDisabled == g_ethernet_link[0].admin_state) {
      g_ethernet_link[0].interface_state = kEthLinkIfStateDisabled;
    } else {
      g_ethernet_link[0].interface_state = link_up ? kEthLinkIfStateEnabled
                                                   : kEthLinkIfStateUnknown;
    }
  }
}

void CipEthernetLinkUpdateInterfaceCounters(CipUdint in_octets,
                                            CipUdint in_ucast,
                                            CipUdint in_nucast,
...
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
  if(OPENER_ETHLINK_INSTANCE_CNT > 0U) {
    g_ethernet_link[0].interface_cntrs.ul.in_octets = in_octets;
    g_ethernet_link[0].interface_cntrs.ul.in_ucast = in_ucast;
    g_ethernet_link[0].interface_cntrs.ul.in_nucast = in_nucast;
    g_ethernet_link[0].interface_cntrs.ul.in_discards = in_discards;
    g_ethernet_link[0].interface_cntrs.ul.in_errors = in_errors;
    g_ethernet_link[0].interface_cntrs.ul.in_unknown_protos = in_unknown_protos;
    g_ethernet_link[0].interface_cntrs.ul.out_octets = out_octets;
    g_ethernet_link[0].interface_cntrs.ul.out_ucast = out_ucast;
    g_ethernet_link[0].interface_cntrs.ul.out_nucast = out_nucast;
    g_ethernet_link[0].interface_cntrs.ul.out_discards = out_discards;
    g_ethernet_link[0].interface_cntrs.ul.out_errors = out_errors;
  }
#else
  (void)in_octets;
  (void)in_ucast;
...
{
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
  const CipEthernetLinkInterfaceCounters *const interface_counters =
    (const CipEthernetLinkInterfaceCounters *)data;
  for (size_t i = 0; i < 11; i++) {
    EncodeCipUdint(&interface_counters->cntr32[i], outgoing_message);
  }
#else
...
{
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
  const CipEthernetLinkMediaCounters *const media_counters =
    (const CipEthernetLinkMediaCounters *)data;
  for (size_t i = 0; i < 12; i++) {
    EncodeCipUdint(&media_counters->cntr32[i], outgoing_message);
  }
#else
  /* Suppress unused parameter compiler warning. */
  (void)data;
                                               ...
                                               ENIPMessage *const outgoing_message)
{
  ...
  while(selected) { /* count # of bits set */
    selected = (uint16_t)(selected & (uint16_t)(selected - 1U));        /* clear the least significant bit set */
    count++;
  }
...
EipStatus GetAndClearEthernetLink(
  ...
  if ( (NULL != attribute) && (NULL != attribute->data) ) {
    ...
    switch (attribute_number) {
      ...
      case 5:
      {
        EipStatus status = GetAttributeSingle(instance,
                                              message_router_request,
                                              message_router_response,
                                              originator_address,
                                              encapsulation_session);
        if( (kEipStatusOkSend == status) &&
            (kCipErrorSuccess == message_router_response->general_status) ) {
          OpenerEthernetLinkPlatformClearCounters(instance->instance_number,
                                                  attribute_number);
        }
        return status;
      }
