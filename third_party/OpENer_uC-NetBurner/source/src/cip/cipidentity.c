...
void SetDeviceProductName(const char *product_name) {
  ...
  /* Keep product name printable and trimmed to avoid control characters from
   * externally supplied values. */
  const unsigned char *begin = (const unsigned char *) product_name;
  ...
  size_t out = 0;
  for(const unsigned char *it = begin;
      it < end && out < (sizeof(sanitized_name) - 1U);
      ++it) {
    sanitized_name[out++] = isprint(*it) ? (char) *it : '_';
  }
  sanitized_name[out] = '\0';
  if(0U == out) {
    return;
  }

  SetCipShortStringByCstr(&g_identity.product_name, sanitized_name);
}
...
static inline void MergeStatusAndExtStatus(void) {
  const CipWord ext_status_mask = (CipWord)kExtStatusMask;
  CipWord status_flags = (CipWord)(g_identity.status & (CipWord)(~ext_status_mask));
  CipWord ext_status = (CipWord)(g_identity.ext_status & ext_status_mask);
  ...
  if(0 !=
    ...
    ext_status = (CipWord)kMajorFault;
  }
...
void CipIdentitySetStatusFlags(const CipWord status_flags) {
  const CipWord ext_status_mask = (CipWord)kExtStatusMask;
  g_identity.status |= (CipWord)(status_flags & (CipWord)(~ext_status_mask));
...
void CipIdentityClearStatusFlags(const CipWord status_flags) {
  const CipWord ext_status_mask = (CipWord)kExtStatusMask;
  g_identity.status =
    (CipWord)(g_identity.status &
              (CipWord)(~(status_flags & (CipWord)(~ext_status_mask))));
  MergeStatusAndExtStatus();
}
...
void CipIdentitySetState(CipIdentityState state) {
  g_identity.state = (CipUsint)state;
}

void CipIdentityEnterOperationalState(void) {
  CipIdentitySetState(kStateOperational);
...
static EipStatus IdentityGetAttributeAll(
  CipInstance *RESTRICT const instance,
  CipMessageRouterRequest *const message_router_request,
  CipMessageRouterResponse *const message_router_response,
  const struct sockaddr *originator_address,
  const CipSessionHandle encapsulation_session) {
  if(0U == instance->instance_number) {
    return GetAttributeAll(instance,
  ...
  EncodeCipUint(&g_identity.vendor_id, &message_router_response->message);
  EncodeCipUint(&g_identity.device_type, &message_router_response->message);
  EncodeCipUint(&g_identity.product_code, &message_router_response->message);
  EncodeRevision(&g_identity.revision, &message_router_response->message);
  EncodeCipWord(&g_identity.status, &message_router_response->message);
  EncodeCipUdint(&g_identity.serial_number, &message_router_response->message);
  EncodeCipShortString(&g_identity.product_name, &message_router_response->message);
  if(0U != (message_router_response->message.used_message_length & 1U)) {
    AddSintToMessage(0, &message_router_response->message);
  }
  EncodeCipUsint(&g_identity.state, &message_router_response->message);
...
EipStatus CipIdentityInit() {
  ...
  CipClass *class = CreateCipClass(kCipIdentityClassCode, 0, /* # of non-default class attributes */
                                   ...
                                   2, /* # class revision — attribute 8 (State) */
  ...
  InsertService(class,
                ...
                &GetAttributeSingle,
                "GetAttributeSingle");
  InsertService(class, kGetAttributeAll, &IdentityGetAttributeAll, "GetAttributeAll");
