...
static bool IsSocketUsedByOtherConnection(const CipConnectionObject *const connection_object,
  ...
  while(NULL != node) {
    const CipConnectionObject *const current_connection = node->data;
    if(current_connection != connection_object) {
      if( (current_connection->socket[kUdpCommuncationDirectionConsuming] == socket_handle)
          || (current_connection->socket[kUdpCommuncationDirectionProducing] == socket_handle) ) {
        return true;
      }
    }
    node = node->next;
        ...
        io_connection_object) )
  {
    if( 256 ==
      ...
      ConnectionObjectSetProductionInhibitTime(io_connection_object,
                                               (CipUint)(ConnectionObjectGetTToORequestedPacketInterval(
                                                 io_connection_object) / 4000U));
    } else {
...
EipUint16 SetupIoConnectionOriginatorToTargetConnectionPoint(
  ...
  if( NULL !=
    ...
    int data_size = (int)ConnectionObjectGetOToTConnectionSize(io_connection_object);
    int diff_size = 0;
    ...
    if( ( (CipByteArray *) attribute->data )->length != data_size ) {
      ...
      connection_object->correct_originator_to_target_size =
        (CipInt)(( (CipByteArray *) attribute->data )->length + diff_size);
...
EipUint16 SetupIoConnectionTargetToOriginatorConnectionPoint(
  ...
  if( NULL !=
    ...
    int data_size = (int)ConnectionObjectGetTToOConnectionSize(io_connection_object);
    int diff_size = 0;
    /* an assembly object should always have a data attribute. */
...
EipStatus OpenConsumingPointToPointConnection(
  ...
  sock_addr_info->sin_family = (CipInt)htons(AF_INET);
                                  ...
                                  CipCommonPacketFormatData *common_packet_format_data)
{
  ...
  socket_address.sin_family = (short)ntohs(
    (uint16_t)common_packet_format_data->address_info_item[j].sin_family);
...
EipStatus SendConnectedData(CipConnectionObject *connection_object) {
  EipStatus send_status = kEipStatusError;
  ...
  if(s_produce_run_idle && !is_heartbeat) {
    common_packet_format_data->data_item.length =
      (EipUint16)(common_packet_format_data->data_item.length + 4U);
  }
  ...
  {
    common_packet_format_data->data_item.length =
      (EipUint16)(common_packet_format_data->data_item.length + 2U);
  ...
  send_status = SendUdpData(&connection_object->remote_address,
                            &outgoing_message);
  return send_status;
}
...
EipStatus HandleReceivedIoConnectionData(CipConnectionObject *connection_object,
      ...
      ConnectionObjectGetTransportClassTriggerTransportClass(connection_object) )
  {
    ...
    data_length = (EipUint16)(data_length - 2U);
  }
...
/* use local CPF data to avoid shared mutable state across connections */
  CipCommonPacketFormatData *common_packet_format_data =
    &cpf_data;
...
void CloseCommunicationChannelsAndRemoveFromActiveConnectionsList(
  ...
  if(kEipInvalidSocket !=
     connection_object->socket[kUdpCommuncationDirectionConsuming]) {
    /* agsweeney@gmail.com: keep shared UDP socket open while other IO connections use it */
    if(!IsSocketUsedByOtherConnection(connection_object,
                                      connection_object->socket[kUdpCommuncationDirectionConsuming])) {
