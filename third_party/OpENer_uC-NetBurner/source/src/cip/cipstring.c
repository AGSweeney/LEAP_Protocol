  ...
  if(0 != str_len) {
    ...
    if(NULL == cip_string->string) {
      ...
      const size_t byte_count =
        (size_t)cip_string->length * (size_t)cip_string->size * sizeof(CipOctet);
...
CipString2 *SetCipString2ByData(CipString2 *const cip_string,
  ...
  if(0 != str_len) {
    ...
    if(NULL == cip_string->string) {
      ...
      cip_string->length = str_len;
      const size_t byte_count = (size_t)str_len * 2U * sizeof(CipOctet);
      memcpy(cip_string->string, data, byte_count);
    }
  }
  return result;
}
