...
/* private functions */
EipStatus ForwardOpen(CipInstance *RESTRICT const instance,
                      CipMessageRouterRequest *const message_router_request,
                      CipMessageRouterResponse *const message_router_response,
                      const struct sockaddr *originator_address,
                      const CipSessionHandle encapsulation_session);

EipStatus LargeForwardOpen(CipInstance *RESTRICT const instance,
                           CipMessageRouterRequest *const message_router_request,
                           CipMessageRouterResponse *const message_router_response,
                           const struct sockaddr *originator_address,
                           const CipSessionHandle encapsulation_session);

EipStatus ForwardClose(CipInstance *RESTRICT const instance,
                       CipMessageRouterRequest *const message_router_request,
                       CipMessageRouterResponse *const message_router_response,
                       const struct sockaddr *originator_address,
                       const CipSessionHandle encapsulation_session);

EipStatus GetConnectionOwner(CipInstance *RESTRICT const instance,
                             CipMessageRouterRequest *const message_router_request,
                             CipMessageRouterResponse *const message_router_response,
                             const struct sockaddr *originator_address,
                             const CipSessionHandle encapsulation_session);

EipStatus GetConnectionData(CipInstance *RESTRICT const instance,
                            CipMessageRouterRequest *const message_router_request,
                            CipMessageRouterResponse *const message_router_response,
                            const struct sockaddr *originator_address,
                            const CipSessionHandle encapsulation_session);

EipStatus SearchConnectionData(CipInstance *RESTRICT const instance,
                               CipMessageRouterRequest *const message_router_request,
                               CipMessageRouterResponse *const message_router_response,
                               const struct sockaddr *originator_address,
                               const CipSessionHandle encapsulation_session);
...
unsigned int GetPaddedLogicalPath(const EipUint8 **logical_path_segment) {
  ...
  if( (padded_logical_path & 3) == 0 ) {
    ...
    padded_logical_path = *(*logical_path_segment)++;
    padded_logical_path |= ((unsigned int)*(*logical_path_segment)++) << 8;
  } else {
...
EipStatus ConnectionManagerInit(EipUint16 unique_connection_id) {
  ...
  if(connection_manager == NULL) {
    return (EipUint8)kEipStatusError;
  }
...
EipStatus HandleReceivedConnectedData(const EipUint8 *const data,
  ...
  /* agsweeney@gmail.com: use per-call CPF storage to avoid shared parser state races. */
    ...
    if( (cpf_data.address_item.type_id ==
      ...
      if(cpf_data.data_item.type_id ==
        ...
        if(connection_object->originator_address.sin_addr.s_addr ==
          ...
          if(SEQ_GT32(cpf_data.address_item.data.
                      sequence_number,
...
EipStatus HandleNonNullNonMatchingForwardOpenRequest(
  ...
  EipUint8 temp = ParseConnectionPath(&g_dummy_connection_object,
                                      message_router_request,
                                      &connection_status);
...
EipStatus ForwardOpenRoutine(CipInstance *instance,
  ...
  size_t minimum_request_size = g_kForwardOpenHeaderLength;
  if(g_dummy_connection_object.is_large_forward_open) {
    minimum_request_size = g_kLargeForwardOpenHeaderLength;
  }
  /* agsweeney@gmail.com: validate fixed ForwardOpen header before parsing fields */
...
EipStatus GetConnectionOwner(CipInstance *RESTRICT const instance,
  ...
  CIPServiceCode service_code = kGetConnectionOwner;
  message_router_response->reply_service = (CipUsint)(0x80U | service_code);

  const size_t kConnectionTriadSize = sizeof(EipUint16) + sizeof(EipUint16) +
                                      sizeof(EipUint32);
  if(message_router_request->request_data_size < kConnectionTriadSize) {
    message_router_response->general_status = kCipErrorNotEnoughData;
    return kEipStatusOk;
  }

  /* Connection triad from request. */
  EipUint16 connection_serial_number = GetUintFromMessage(
    &message_router_request->data);
  EipUint16 originator_vendor_id = GetUintFromMessage(
    &message_router_request->data);
  ...
  while(NULL != iterator) {
    CipConnectionObject *candidate = iterator->data;
    if( (candidate->connection_serial_number == connection_serial_number) &&
        (candidate->originator_vendor_id == originator_vendor_id) &&
        (candidate->originator_serial_number == originator_serial_number) ) {
      connection_object = candidate;
      break;
    }
...
void AssembleConnectionDataResponseMessage(
  ...
  AddDintToMessage((EipUint32)connection_object->transmission_trigger_timer,
                   &message_router_response->message);
        ...
        switch(extended_status) {
            ...
            kConnectionManagerExtendedStatusCodeErrorInvalidOToTConnectionSize:
          {
            ...
            message_router_response->additional_status[1] =
              (EipUint16)connection_object->correct_originator_to_target_size;
            break;
          }
            ...
            kConnectionManagerExtendedStatusCodeErrorInvalidTToOConnectionSize:
          {
            ...
            message_router_response->additional_status[0] = extended_status;
            message_router_response->additional_status[1] =
              (EipUint16)connection_object->correct_target_to_originator_size;
            break;
          }
          ...
          case kSegmentTypeDataSegment: {
            ...
            switch(data_segment_type) {
              case kDataSegmentSubtypeSimpleData:
                g_config_data_length = (unsigned int)message[1] * 2U; /*data segments store length 16-bit word wise */
            ...
            switch(subtype) {
              case kNetworkSegmentSubtypeProductionInhibitTimeInMilliseconds:
                if(kConnectionObjectTransportClassTriggerProductionTriggerCyclic
                  ...
                  *extended_error = (EipUint16)(connection_path_size - remaining_path); /*offset in 16Bit words where within the connection path the error happened*/
