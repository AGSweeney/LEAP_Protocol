  ...
  while(index < path_bytes) {
    ...
    if(logical_type == LOGICAL_SEGMENT_TYPE_SPECIAL) {
      ...
      const CipUint vendor_id =
        (CipUint)((CipUint)path_data[index + 2] |
                  (CipUint)((CipUint)path_data[index + 3] << 8));
      const CipUint device_type =
        (CipUint)((CipUint)path_data[index + 4] |
                  (CipUint)((CipUint)path_data[index + 5] << 8));
      const CipUint product_code =
        (CipUint)((CipUint)path_data[index + 6] |
                  (CipUint)((CipUint)path_data[index + 7] << 8));
