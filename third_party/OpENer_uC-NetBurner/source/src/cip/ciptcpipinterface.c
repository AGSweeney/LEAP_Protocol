...
#define CFG_CAPS_ACD_CAPABLE          kTcpipCfgCapsAcdCapable
 ...
 */
static bool IsInClassAbc(CipUdint ip_addr) {
  CipUdint ip = ntohl(ip_addr);
  const bool class_a = ((ip & 0x80000000UL) == 0U);
  const bool class_b = ((ip & 0xC0000000UL) == 0x80000000UL);
  const bool class_c = ((ip & 0xE0000000UL) == 0xC0000000UL);
 ...
 */
static bool IsOnLoopbackNetwork(CipUdint ip_addr) {
  CipUdint ip = ntohl(ip_addr);
 ...
 */
static bool IsNetworkOrBroadcastIp(CipUdint ip_addr,
                                   CipUdint net_mask) {
...
void EncodeCipLastConflictDetected(const void *const data,
                                   ENIPMessage *const outgoing_message) {
#if defined(OPENER_NB_ACD) && OPENER_NB_ACD
  OpenerNbAcdEncodeLastConflict(data, outgoing_message);
#else
  /* Suppress unused parameter compiler warning. */
  (void)data;

  const unsigned int kAttribute11Size = (unsigned int)(sizeof(CipUsint) +
                                                       6U * sizeof(CipUsint) +
                                                       28U * sizeof(CipUsint));
...
int DecodeTcpIpInterfaceConfigurationControl( /* Attribute 3 */
	...
	CipDword configuration_control_received = GetDwordFromMessage(
	...
	if ((configuration_control_received & kTcpipCfgCtrlMethodMask) >= 0x03
		...
		const CipDword previous_configuration_control = *(CipDword *)data;
		*(CipDword *)data = configuration_control_received;
		if(previous_configuration_control != configuration_control_received) {
			/* Config control changes become active only after reset/apply. */
...
int DecodeCipTcpIpInterfaceConfiguration( /* Attribute 5 */
		void *const data, //kCipUdintUdintUdintUdintUdintString
		CipMessageRouterRequest *const message_router_request,
		CipMessageRouterResponse *const message_router_response) {

	int number_of_decoded_bytes = -1;
	CipTcpIpInterfaceConfiguration *const typed_data =
			(CipTcpIpInterfaceConfiguration *const)data;
	...
	EipByte domain_name_buf[49] = { 0 };
	if (domain_name_length > 0) {
		memcpy(domain_name_buf, message_router_request->data, domain_name_length);
	}
	SetCipStringByData(&if_cfg.domain_name, domain_name_length,
			message_router_request->data);
	domain_name_length = (CipUint)(((CipUdint)domain_name_length + 1UL) &
	                               (~(CipUdint)0x0001UL)); /* Align for possible pad byte */
	...
	if (!IsValidNetworkConfig(&if_cfg)
			|| (domain_name_length > 0
					&& !IsValidDomain(domain_name_buf))) {
		message_router_response->general_status =
				kCipErrorInvalidAttributeValue;
		return number_of_decoded_bytes;
	}

	*typed_data = if_cfg; //write data to attribute
...
int DecodeCipTcpIpInterfaceHostName( /* Attribute 6 */
	...
	int number_of_decoded_bytes = -1;
	CipString *const typed_data = (CipString *const)data;
	          ...
	          EipByte host_name_buf[65] = { 0 };
	          if (host_name_length > 0) {
	            memcpy(host_name_buf, message_router_request->data, host_name_length);
	          }
	          SetCipStringByData(&tmp_host_name,
	                             host_name_length,
	                             message_router_request->data);
	          host_name_length = (CipUint)(((CipUdint)host_name_length + 1UL) &
	                                       (~(CipUdint)0x0001UL));  /* Align for possible pad byte */
	          OPENER_TRACE_INFO("Host Name: ds %hu '%s'\n",
	                            host_name_length,
	                            host_name_buf);

	          if (!IsValidNameLabel(host_name_buf) ) {
	            message_router_response->general_status =
	              kCipErrorInvalidAttributeValue;
	            return number_of_decoded_bytes;
	          }

	          *typed_data = tmp_host_name; //write data to attribute
...
#if defined(OPENER_NB_ACD) && OPENER_NB_ACD
                  kCipBool,
                  OpenerNbAcdEncodeLastConflict,
                  OpenerNbAcdDecodeClearLastConflict,
#else
                  kCipBool,
