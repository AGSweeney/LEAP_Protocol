...
EipStatus CreateFileObject(unsigned int instance_nr,
  ...
  CipInstance *instance = GetCipInstance(file_object_class,
                                         (CipInstanceNum)instance_nr);
                                   ...
                                   CipMessageRouterResponse *const message_router_response)
{
  ...
  if (message_router_request->request_data_size > 0) {       //check if message contains data
    ...
    const CipOctet *message_data = message_router_request->data;             //get message data pointer
...
/** @brief Update file length metadata from current file handle.
 *
 * @param file_instance File object instance to update.
 */
void CipFileSetFileLength(CipFileObjectValues *const file_instance) {
  fseek(file_instance->file_handle, 0L, SEEK_END);
  file_instance->file_size = ftell(file_instance->file_handle);
  rewind(file_instance->file_handle);
}

/** @brief Recalculate and store 16-bit additive checksum for a file instance.
 *
 * @param file_instance File object instance to update.
 */
void CipFileSetChecksum(CipFileObjectValues *const file_instance) {
  CipUint checksum = 0;
  for(size_t i = 0; i < file_instance->file_size; ++i) {
    CipOctet byte = 0;
    if( 1 !=
        fread(&byte, sizeof(CipOctet), 1, file_instance->file_handle) ) {
      OPENER_TRACE_ERR("File read error in checksum creation!\n");
    }
    checksum += byte;
  }
  file_instance->file_checksum =
...
void CipFileSetInstanceName(CipFileObjectValues *const file_instance,
  ...
  CipShortString *instance_name_short_string =
    (CipShortString *) (file_instance->instance_name.array_of_string_i_structs[0
                        ...
                        string);
  if(0U == instance_name_string_size) {
    instance_name_short_string->length = 0U;
  } else if(instance_name_string_size > 0xFFU) {
    instance_name_short_string->length = 0xFFU;
  } else {
    instance_name_short_string->length = (EipUint8)(instance_name_string_size - 1U);
  }
