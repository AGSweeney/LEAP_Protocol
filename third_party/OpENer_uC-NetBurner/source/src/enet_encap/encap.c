...
EipStatus HandleReceivedUnregisterSessionCommand(const int socket, const EncapsulationData *const receive_data, ENIPMessage *const outgoing_message);

EipStatus HandleReceivedSendUnitDataCommand(const int socket, const EncapsulationData *const receive_data, const struct sockaddr *const originator_address,
...
SessionStatus CheckRegisteredSessions(const int socket, const EncapsulationData *const receive_data);
...
EipStatus HandleReceivedExplictTcpData(int socket, EipUint8 *buffer, size_t length, int *number_of_remaining_bytes, struct sockaddr *originator_address,
  ...
  const int remaining_bytes =
    (int)CreateEncapsulationStructure(buffer, length, &encapsulation_data);
    ...
    if(*number_of_remaining_bytes >= 0) /* check if the message is corrupt: header size + claimed payload size > than what we actually received*/
    {
      ...
      switch(encapsulation_data.command_code){
        ...
        case (kEncapsulationCommandUnregisterSession):
          ...
          /* agsweeney@gmail.com: enforce socket/session ownership checks in handlers */
          return_value = HandleReceivedUnregisterSessionCommand(socket, &encapsulation_data, outgoing_message);
          break;

        case (kEncapsulationCommandSendRequestReplyData):
          OPENER_TRACE_INFO("Send Request/Reply Data\n");
          return_value = HandleReceivedSendRequestResponseDataCommand(socket, &encapsulation_data, originator_address, outgoing_message);
          break;

        case (kEncapsulationCommandSendUnitData):
          OPENER_TRACE_INFO("Send Unit Data\n");
          return_value = HandleReceivedSendUnitDataCommand(socket, &encapsulation_data, originator_address, outgoing_message);
          break;
...
EipStatus HandleReceivedExplictUdpData(const int socket, const struct sockaddr_in *from_address, const EipUint8 *buffer, const size_t buffer_length,
  ...
  /* returns how many bytes are left after the encapsulated data*/
  const int remaining_bytes =
    (int)CreateEncapsulationStructure(buffer, buffer_length, &encapsulation_data);
...
void GenerateEncapsulationHeader(const EncapsulationData *const receive_data, const size_t command_specific_data_length, const CipSessionHandle session_handle,
  ...
  AddIntToMessage((EipUint16)command_specific_data_length, outgoing_message);
  ...
  /* agsweeney@gmail.com: guard sender-context memcpy against header buffer exhaustion. */
  if(sizeof(outgoing_message->message_buffer) - outgoing_message->used_message_length < kSenderContextSize) {
    OPENER_TRACE_ERR("encap.c: insufficient buffer space for encapsulation sender context\n");
    return;
  }
  memcpy(outgoing_message->current_message_position,
         receive_data->sender_context,
         (size_t)kSenderContextSize); // sender context
  outgoing_message->current_message_position += kSenderContextSize;
  outgoing_message->used_message_length += (size_t)kSenderContextSize;
...
CipUint ListIdentityGetCipIdentityItemLength() {
  const size_t identity_item_length =
    sizeof(CipUint) + sizeof(CipInt) + sizeof(CipUint) + sizeof(CipUdint) +
    8U * sizeof(CipUsint) + sizeof(CipUint) + sizeof(CipUint) +
    sizeof(CipUint) + 2U * sizeof(CipUsint) + sizeof(CipWord) +
    sizeof(CipUdint) + sizeof(CipUsint) +
    (size_t)g_identity.product_name.length + sizeof(CipUsint);
  return (CipUint)identity_item_length;
}
...
void EncodeListIdentitySecurityItem(ENIPMessage *const outgoing_message) {
  const CipUint kCipSecurityItemId = 0x0086U;
  const CipUint kCipSecurityItemLength = 7U;
  ...
  AddSintToMessage(0, outgoing_message); /* IANA port state placeholder */
...
void EncapsulateListIdentityResponseMessage(const EncapsulationData *const receive_data, ENIPMessage *const outgoing_message) {
  ...
  CipUint list_identity_length = (CipUint)(ListIdentityGetCipIdentityItemLength() +
                                 sizeof(CipUint) + sizeof(CipUint) + sizeof(CipUint)); /* Last element is item count */
#if defined(CIP_SECURITY_OBJECTS) && 0 != CIP_SECURITY_OBJECTS
  list_identity_length += sizeof(CipUint) + sizeof(CipUint) + 7U;
#endif

  GenerateEncapsulationHeader(receive_data, list_identity_length, 0,
  /* Session handle will be ignored by receiver */
  kEncapsulationProtocolSuccess, outgoing_message);

#if defined(CIP_SECURITY_OBJECTS) && 0 != CIP_SECURITY_OBJECTS
  AddIntToMessage(2, outgoing_message); /* Item count: two items */
#else
...
void HandleReceivedRegisterSessionCommand(int socket, const EncapsulationData *const receive_data, ENIPMessage *const outgoing_message) {
  ...
  /* agsweeney@gmail.com: enforce Register Session command-specific payload length */
  if(receive_data->data_length != kRegisterSessionExpectedDataLength) {
    OPENER_TRACE_WARN("encap.c: invalid Register Session data length: %u (expected %u)\n",
                      receive_data->data_length,
                      kRegisterSessionExpectedDataLength);
    EncapsulateRegisterSessionCommandResponseMessage(receive_data, 0, kEncapsulationProtocolInvalidLength, outgoing_message);
    return;
  }
 ...
 * @param socket TCP socket that issued the command.
...
EipStatus HandleReceivedSendUnitDataCommand(const int socket, const EncapsulationData *const receive_data, const struct sockaddr *const originator_address,
  ...
  if(receive_data->data_length >= 6) {
    ...
    ((EncapsulationData* const ) receive_data)->data_length =
      (CipUint)(((EncapsulationData* const ) receive_data)->data_length - 6U); /* the rest is in CPF format*/

    if(kSessionStatusValid == CheckRegisteredSessions(socket, receive_data)) /* see if the EIP session is registered*/
    {
 ...
 */
EipStatus HandleReceivedSendRequestResponseDataCommand(const int socket, const EncapsulationData *const receive_data, const struct sockaddr *const originator_address,
    ENIPMessage *const outgoing_message) {
  EipStatus return_value = kEipStatusOkSend;
  /* EipStatus*/return_value = kEipStatusOk; /* TODO: Shouldn't this be kEipStatusOk cause we must not send any response if data_length < 6? */
                                          ...
                                          EncapsulationData *const encapsulation_data)
{
  ...
  memcpy(encapsulation_data->sender_context, receive_buffer, (size_t)kSenderContextSize);
...
SessionStatus CheckRegisteredSessions(const int socket, const EncapsulationData *const receive_data) {
  ...
  if((0 < receive_data->session_handle) && (receive_data->session_handle <=
  OPENER_NUMBER_OF_SUPPORTED_SESSIONS)) {
    /* agsweeney@gmail.com: session handle must belong to requesting socket */
...
void CloseSessionBySessionHandle(const CipConnectionObject *const connection_object) {
  ...
  /* agsweeney@gmail.com: reject out-of-range session handles before array access. */
  if(session_handle < 1 || session_handle > OPENER_NUMBER_OF_SUPPORTED_SESSIONS) {
    OPENER_TRACE_WARN("encap.c: invalid session handle %lu for close-by-handle\n",
...
void ManageEncapsulationMessages(const MilliSeconds elapsed_time) {
  for(size_t i = 0; i < ENCAP_NUMBER_OF_SUPPORTED_DELAYED_ENCAP_MESSAGES; i++) {
    if(kEipInvalidSocket != g_delayed_encapsulation_messages[i].socket) {
      g_delayed_encapsulation_messages[i].time_out -= (EipInt32)elapsed_time;
      if(0 >= g_delayed_encapsulation_messages[i].time_out) {
        /* If delay is reached or passed, send the UDP message */
        if(g_delayed_encapsulation_messages[i].outgoing_message.used_message_length >
           (size_t)INT_MAX) {
          OPENER_TRACE_ERR("encap.c: delayed UDP packet too large: %zu\n",
                           g_delayed_encapsulation_messages[i].outgoing_message.used_message_length);
          g_delayed_encapsulation_messages[i].socket = kEipInvalidSocket;
          continue;
        }
        OpenerHalEndpoint dest_endpoint = {0};
        OpenerSockaddrInToEndpoint(&g_delayed_encapsulation_messages[i].receiver,
                                   &dest_endpoint);
        if(OpenerHal_UdpSend(g_delayed_encapsulation_messages[i].socket,
                             &dest_endpoint,
                             g_delayed_encapsulation_messages[i].outgoing_message.message_buffer,
                             g_delayed_encapsulation_messages[i].outgoing_message.used_message_length)
           != kOpenerHalOk) {
          OPENER_TRACE_ERR("encap.c: delayed UDP send failed\n");
        }
        g_delayed_encapsulation_messages[i].socket = kEipInvalidSocket;
      }
    }
...
void CloseEncapsulationSessionBySockAddr(const CipConnectionObject *const connection_object) {
  for(size_t i = 0; i < OPENER_NUMBER_OF_SUPPORTED_SESSIONS; ++i) {
    if(kEipInvalidSocket != g_registered_sessions[i]) {
      ...
      if(OpenerHal_GetPeer(g_registered_sessions[i], &peer_endpoint) != kOpenerHalOk) {
        ...
        OpenerEndpointToSockaddrIn(&peer_endpoint, &encapsulation_session_addr);
      }
      /* agsweeney@gmail.com: match both peer IP and TCP source port to avoid closing unrelated reconnect sessions */
      if( (encapsulation_session_addr.sin_addr.s_addr == connection_object->originator_address.sin_addr.s_addr)
          && (encapsulation_session_addr.sin_port == connection_object->originator_address.sin_port) ) {
