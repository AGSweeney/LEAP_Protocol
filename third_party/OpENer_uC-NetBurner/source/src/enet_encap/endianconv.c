...
static bool HasMessageCapacity(const ENIPMessage *const outgoing_message,
                               const size_t needed_bytes) {
  return (outgoing_message->used_message_length + needed_bytes) <=
...
CipInt GetIntFromMessage(const EipUint8 **const buffer) {
  const unsigned char *const buffer_address = (unsigned char *) *buffer;
  /* agsweeney@gmail.com: cast before shifting to avoid signed-overflow UB */
  EipUint16 data = (EipUint16)((EipUint16)buffer_address[0] |
                               (EipUint16)((EipUint16)buffer_address[1] << 8));
  *buffer += 2;
  return (CipInt)data;
}
...
CipUint GetUintFromMessage(const CipOctet **const buffer_address) {
  const CipOctet *buffer = *buffer_address;
  EipUint16 data = (EipUint16)((EipUint16)buffer[0] |
                               (EipUint16)((EipUint16)buffer[1] << 8));
  *buffer_address += 2;
  return data;
}
...
CipUdint GetUdintFromMessage(const CipOctet **const buffer_address) {
  const CipOctet *buffer = *buffer_address;
  CipUdint data = (CipUdint) buffer[0] |
                  ( (CipUdint) buffer[1] << 8 ) |
                  ( (CipUdint) buffer[2] << 16 ) |
                  ( (CipUdint) buffer[3] << 24 );
  *buffer_address += 4;
  return data;
}
...
CipUdint GetDwordFromMessage(const CipOctet **const buffer_address) {
  const CipOctet *buffer = *buffer_address;
  CipDword data = (CipDword) buffer[0] |
                  ( (CipDword) buffer[1] << 8 ) |
                  ( (CipDword) buffer[2] << 16 ) |
                  ( (CipDword) buffer[3] << 24 );
  *buffer_address += 4;
  return data;
}
...
void AddSintToMessage(const EipUint8 data,
  ...
  /* agsweeney@gmail.com: guard ENIPMessage writes against buffer overflow */
  if(!HasMessageCapacity(outgoing_message, 1)) {
    return;
  }
...
void AddIntToMessage(const EipUint16 data,
                     ENIPMessage *const outgoing_message) {
  if(!HasMessageCapacity(outgoing_message, 2)) {
    return;
  }
...
void AddDintToMessage(const EipUint32 data,
                      ENIPMessage *const outgoing_message) {
  if(!HasMessageCapacity(outgoing_message, 4)) {
    return;
  }
  outgoing_message->current_message_position[0] = (unsigned char) data;
  outgoing_message->current_message_position[1] = (unsigned char) (data >> 8);
...
void AddLintToMessage(const EipUint64 data,
                      ENIPMessage *const outgoing_message) {
  if(!HasMessageCapacity(outgoing_message, 8)) {
    return;
  }
...
void MoveMessageNOctets(const int amount_of_bytes_moved,
                        ENIPMessage *const outgoing_message) {
  if(amount_of_bytes_moved <= 0) {
    return;
  }
  if(!HasMessageCapacity(outgoing_message, (size_t) amount_of_bytes_moved)) {
    /* agsweeney@gmail.com: clamp cursor movement to ENIP message buffer bounds */
    outgoing_message->current_message_position =
      outgoing_message->message_buffer + PC_OPENER_ETHERNET_BUFFER_SIZE;
  ...
  outgoing_message->used_message_length += (size_t)amount_of_bytes_moved;
}

void FillNextNMessageOctetsWith(CipOctet value,
                                unsigned int amount_of_bytes_written,
                                ENIPMessage *const outgoing_message) {
  if(!HasMessageCapacity(outgoing_message, (size_t)amount_of_bytes_written)) {
    return;
  }
                                                          ...
                                                          ENIPMessage *const outgoing_message)
{
  ...
  MoveMessageNOctets((int)amount_of_filled_bytes, outgoing_message);
}
