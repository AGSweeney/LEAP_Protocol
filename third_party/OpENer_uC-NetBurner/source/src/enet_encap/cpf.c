...
EipStatus NotifyCommonPacketFormat(const EncapsulationData *const received_data,
                                             ...
                                             &cpf_data) ) )
  {
    ...
    if(cpf_data.address_item.type_id ==
  ...
  if(kEipStatusError == return_value) {
    ...
    { /* ConnectedAddressItem item */
      CipConnectionObject *connection_object = GetConnectedObject(
        cpf_data.address_item.data.connection_identifier);
      if(NULL != connection_object) {
        ...
        if(cpf_data.data_item.type_id ==
          ...
          EipUint8 *buffer = cpf_data.data_item.data;
          /* agsweeney@gmail.com: require room for sequence number before decoding it. */
          if(cpf_data.data_item.length < sizeof(EipUint16)) {
            OPENER_TRACE_ERR("notifyConnectedCPF: connected data item too short for sequence number\n");
            return_value = kEipStatusError;
            goto notify_connected_cleanup;
          }
          cpf_data.address_item.data.sequence_number =
             ...
             cpf_data.address_item.data.sequence_number)
          {
            ...
            /* agsweeney@gmail.com: avoid underflow when rebuilding cached reply header. */
            if(outgoing_message->used_message_length < ENCAPSULATION_HEADER_LENGTH) {
              OPENER_TRACE_ERR("notifyConnectedCPF: cached reply shorter than encapsulation header\n");
              return_value = kEipStatusError;
              goto notify_connected_cleanup;
            }
            outgoing_message->used_message_length -= ENCAPSULATION_HEADER_LENGTH;
            GenerateEncapsulationHeader(received_data,
                                        outgoing_message->used_message_length,
                                        received_data->session_handle,
                                        kEncapsulationProtocolSuccess,
          ...
          connection_object->sequence_count_consuming =
            (CipUint)cpf_data.address_item.data.sequence_number;
...
{
#define CPF_REQUIRE_BYTES(required) \
  if( (size_t) (cpf_end - data) < (required) ) { \
  ...
  const EipUint8 *const cpf_end = data + data_length;
  CPF_REQUIRE_BYTES(2);
  CipUint item_count = GetUintFromMessage(&data);
  /* agsweeney@gmail.com: enforce supported CPF item-count range before parsing */
  if(item_count > 4U) {
    return kEipStatusError;
  }
  ...
  if(common_packet_format_data->item_count >= 1U) {
    ...
    if( (0 != common_packet_format_data->address_item.length)
        && (4 != common_packet_format_data->address_item.length)
        && (8 != common_packet_format_data->address_item.length) ) {
      return kEipStatusError;
    }
    length_count += 4;
  ...
  if(common_packet_format_data->item_count >= 2U) {
    CPF_REQUIRE_BYTES(4);
    ...
    if(data_length >=
       length_count + 4U + (size_t)common_packet_format_data->data_item.length) {
        ...
        j++)
    {
      ...
      if( (common_packet_format_data->address_info_item[j].type_id ==
        ...
        if(common_packet_format_data->address_info_item[j].length != 16) {
          return kEipStatusError;
        }
        CPF_REQUIRE_BYTES(16);
        common_packet_format_data->address_info_item[j].sin_family =
          (CipInt)GetUintFromMessage(&data);
        ...
        CPF_REQUIRE_BYTES(common_packet_format_data->address_info_item[j].length);
        data += common_packet_format_data->address_info_item[j].length;
        length_count += common_packet_format_data->address_info_item[j].length;
      }
    }
  }
...
void EncodeDataItemData(
  const CipCommonPacketFormatData *const common_packet_format_data_item,
  ENIPMessage *const outgoing_message) {
  size_t bytes_to_copy = common_packet_format_data_item->data_item.length;
  const size_t remaining_space = PC_OPENER_ETHERNET_BUFFER_SIZE -
                                 outgoing_message->used_message_length;
  if(bytes_to_copy > remaining_space) {
    /* agsweeney@gmail.com: prevent encoded CPF data from overflowing ENIPMessage */
    OPENER_TRACE_ERR("EncodeDataItemData: truncating %zu bytes to %zu bytes\n",
                     bytes_to_copy,
                     remaining_space);
...
void EncodeConnectedDataItemLength(
  ...
  AddIntToMessage( (EipUint16) (message_router_response->message.
                                used_message_length + 4U + 2U                                 /* TODO: Magic numbers */
                                + (2U *
                                   (size_t)message_router_response->
...
void EncodeUnconnectedDataItemLength(
  ...
  ENIPMessage *const outgoing_message) {
  AddIntToMessage( (EipUint16) (message_router_response->message.
                                used_message_length + 4U                                 /* TODO: Magic number */
                                + (2U *
                                   (size_t)message_router_response->
                                   size_of_additional_status) ),
                   outgoing_message );
...
void EncodeMessageRouterResponseData(
  const CipMessageRouterResponse *const message_router_response,
  ENIPMessage *const outgoing_message) {
  size_t bytes_to_copy = message_router_response->message.used_message_length;
  /* agsweeney@gmail.com: clamp response copy to available ENIP message capacity. */
  const size_t remaining_space = sizeof(outgoing_message->message_buffer) -
                                 outgoing_message->used_message_length;
  if(bytes_to_copy > remaining_space) {
    OPENER_TRACE_ERR("EncodeMessageRouterResponseData: truncating data from %u to %u bytes\n",
    ...
    if(message_router_response) {
      ...
      if(common_packet_format_data_item->data_item.type_id ==
        ...
        EncodeConnectedDataItemLength(message_router_response,
                                      outgoing_message);
        EncodeSequenceNumber(common_packet_format_data_item,
                             outgoing_message);
