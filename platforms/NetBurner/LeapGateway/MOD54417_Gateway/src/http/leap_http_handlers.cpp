...
static void http_write_tenths(int sock, uint32_t value)
{
    fdprintf(sock, "%u.%u",
...
static void http_write_json_string(int sock, const char *value)
{
    fdprintf(sock, "\"");
    if (value != nullptr)
    {
        for (const char *p = value; *p != '\0'; ++p)
        {
            ...
            else if (static_cast<unsigned char>(*p) >= 0x20u)
            {
                fdprintf(sock, "%c", *p);
            }
        }
...
static int HandleSystemHealthApi(int sock, HTTP_Request &req)
{
    (void)req;
    const GatewaySystemHealthSnapshot health = GatewaySystemHealthGetSnapshot();

    fdprintf(sock, "HTTP/1.0 200 OK\r\nPragma: no-cache\r\nContent-Type: application/json\r\n\r\n");
    fdprintf(sock, "{\"ready\":%s,\"sample_ms\":%u,",
             health.ready ? "true" : "false",
             static_cast<unsigned>(health.sample_ms));
    fdprintf(sock, "\"cpu_load_pct\":");
    http_write_tenths(sock, health.cpu_load_tenths);
    fdprintf(sock, ",\"idle_pct\":");
    http_write_tenths(sock, health.idle_tenths);
    fdprintf(sock, ",\"leap_task_pct\":");
    http_write_tenths(sock, health.leap_tenths);
    fdprintf(sock, ",\"eip_task_pct\":");
    http_write_tenths(sock, health.eip_tenths);
    fdprintf(sock, ",\"network_task_pct\":");
    http_write_tenths(sock, health.network_tenths);
    fdprintf(sock, ",\"other_task_pct\":");
    http_write_tenths(sock, health.other_tenths);
    fdprintf(sock, ",\"max_task_pct\":");
    http_write_tenths(sock, health.max_task_tenths);
    fdprintf(sock, ",\"max_task_ticks\":%u,\"max_task_name\":",
             static_cast<unsigned>(health.max_task_ticks));
    http_write_json_string(sock, health.max_task_name);
    fdprintf(sock, "}");
    return 1;
}
...
static bool MatchSystemHealthApi(HTTP_Request &req) { return MatchApiPath(req, "api/system/health"); }
static bool MatchSystemHealthV1Api(HTTP_Request &req) { return MatchApiPath(req, "api/v1/system/health"); }
