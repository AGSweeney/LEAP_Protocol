/*
 * leap_transport.h — Linux AF_PACKET transport with the RTEMS gateway API.
 *
 * Drop-in replacement for LeapPort/src/leap_transport.h so the shared
 * LeapGateway sources compile unchanged on Linux. The LeapRtemsTransport
 * name is kept deliberately: gateway_*.c reference it and the RTEMS build
 * must keep working from the same sources.
 *
...
/*
 * Receive one Ethernet frame. Returns 0 on success, -1 on timeout/no data,
 * positive errno-style code on error.
 */
...
/*
 * Send LEAP payload (Ethernet header built here). Returns 0 on success.
 */
