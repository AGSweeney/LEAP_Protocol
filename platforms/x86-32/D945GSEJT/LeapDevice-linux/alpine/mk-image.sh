#!/bin/bash
# Build raw MBR + ext4 Alpine i386 disk image for D945GSEJT LEAP device CF.
...
IMG="${LEAPOS_DEVICE_ALPINE_IMG:-$IMAGE_DIR/leapos-device-alpine.img}"
...
build_rootfs() {
	...
	chmod 755 "$ROOT/usr/sbin/leap-device"

	echo "Rebuilding PXE initramfs (network + r8169 for HTTP modloop) ..."
	chroot "$ROOT" /bin/sh <<'PXE_INITRD'
set -e
KVER="$(apk info -e linux-lts 2>/dev/null | sed -n 's/^linux-lts-\([0-9].*\)$/\1/p' | head -1)"
if [ -z "$KVER" ]; then
	KVER="$(ls -1 /lib/modules 2>/dev/null | grep -v -E 'microsoft|WSL' | head -1)"
fi
if [ -z "$KVER" ] || [ ! -d "/lib/modules/$KVER" ]; then
	echo "error: linux-lts kernel modules not found under /lib/modules" >&2
	ls -la /lib/modules 2>&1 || true
	exit 1
fi
CONF=/etc/mkinitfs/mkinitfs.conf
for feat in network dhcp squashfs; do
	if ! grep -qw "$feat" "$CONF"; then
		sed -i "s/^features=\"/features=\"$feat /" "$CONF"
	fi
done
echo "mkinitfs features: $(grep '^features=' "$CONF")"
...
echo "=== LeapOS-Device Alpine image build ==="
...
if [ "$IMAGE_MB" = "auto" ]; then
	...
	echo "Auto image size: ${IMAGE_MB} MiB"
fi
...
if [ "$ROOT_KB" -gt $((PART_KB * 95 / 100)) ]; then
	...
	echo "Increase IMAGE_MB or trim packages/rootfs." >&2
...
KERNEL_ARGS="root=LABEL=LEAPDEV rootfstype=ext4 rootwait modules=sd-mod,ext4 earlyprintk=serial,ttyS0,115200 console=tty1 console=ttyS0,115200n8"
...
find "$ROOT/etc" "$ROOT/usr/sbin" -type f \
	-exec grep -Iq . {} \; -exec sed -i 's/\r$//' {} \; 2>/dev/null || true
chmod 755 "$ROOT/usr/sbin/leap-device" "$ROOT/etc/init.d/leap-device"
...
enable_svc default leap-device
...
menuentry "LeapOS-Device (Alpine i386, COM1 115200)" {
...
rm -f "$PART_IMG"
echo "Packing rootfs into ext4 (${PART_MIB} MiB, ${FS_BLOCKS} blocks) ..."
mke2fs -q -t ext4 -b 4096 -L LEAPDEV -d "$ROOT" "$PART_IMG" "$FS_BLOCKS"
...
terminal_output serial console
set root=(hd0,msdos1)
echo "LeapOS-Device: loading Alpine kernel..."
linux /boot/vmlinuz-lts ${KERNEL_ARGS}
initrd /boot/initramfs-lts
boot
EOF
