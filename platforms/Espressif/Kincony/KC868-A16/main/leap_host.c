...
#define LEAP_KC_PRODUCT_CODE      0x0868A016u
#define LEAP_KC_FIRMWARE_REVISION 1u
...
static void leap_log_device_config(void)
{
    ...
    ESP_LOGI(TAG,
             "LEAP KC868-A16 protocol %u.%u git=%s built=%s fw_rev=%u",
             (unsigned)LEAP_VERSION_MAJOR,
             (unsigned)LEAP_VERSION_MINOR,
             LEAP_BUILD_GIT,
             LEAP_BUILD_DATE,
             (unsigned)LEAP_KC_FIRMWARE_REVISION);
    ...
    ESP_LOGI(TAG,
             "  locate_caps=0x%04X (GPIO%d IR transmitter)",
             (unsigned)LEAP_LOCATE_FLAG_LED,
             LOCATE_LED_PIN);
    ESP_LOGI(TAG,
             "  PD I/O: %u relay outputs, %u digital inputs (PCF8574)",
...
static int leap_item_needs_input_refresh(const LeapHostRxItem *item)
{
    ...
    if (leap_frame_parse(item->payload, item->payload_length, &view) != LEAP_FRAME_OK) {
        return 0;
    }

    return (view.header.service_id == (uint16_t)LEAP_SERVICE_PD) ? 1 : 0;
}
...
int leap_host_init(struct netif *netif)
{
    ...
    s_stack.dir.config.identity.product_code      = LEAP_KC_PRODUCT_CODE;
    s_stack.dir.config.identity.firmware_revision = LEAP_KC_FIRMWARE_REVISION;
    memcpy(s_stack.disc.config.identity.primary_mac, netif->hwaddr, 6);
    s_stack.disc.config.identity.product_code      = LEAP_KC_PRODUCT_CODE;
    s_stack.disc.config.identity.firmware_revision = LEAP_KC_FIRMWARE_REVISION;
