/**
  ******************************************************************************
  * @file    STM/STM32F746G-DISCO/Src/ethernetif.c
...
static uint8_t s_netif_mac[ETH_HWADDR_LEN];

/* Private function prototypes -----------------------------------------------*/
static void ethernetif_derive_mac(uint8_t mac[ETH_HWADDR_LEN]);
...
static void ethernetif_derive_mac(uint8_t mac[ETH_HWADDR_LEN])
{
  ...
  mac[0] = 0x02u;
  mac[1] = 0x00u;
  mac[2] = (uint8_t)(uid0 & 0xFFu);
  mac[3] = (uint8_t)((uid0 >> 8) & 0xFFu);
  mac[4] = (uint8_t)(uid1 & 0xFFu);
  mac[5] = (uint8_t)(((uid2 & 0xFFu) ^ (uint8_t)((uid2 >> 8) & 0xFFu)) & 0xFFu);
}

static void ethernetif_commit_mac_address(const uint8_t mac[ETH_HWADDR_LEN])
{
  uint32_t high;
  uint32_t low;

  if (mac == NULL || EthHandle.Instance == NULL)
  {
    return;
  }

  high = ((uint32_t)mac[5] << 8) | (uint32_t)mac[4];
  low  = ((uint32_t)mac[3] << 24) | ((uint32_t)mac[2] << 16) |
         ((uint32_t)mac[1] << 8) | (uint32_t)mac[0];

  EthHandle.Instance->MACA0HR = high;
  EthHandle.Instance->MACA0LR = low;
}

static void ethernetif_configure_leap_rx(void)
{
  ETH_MACFilterConfigTypeDef filter;

  if (EthHandle.Instance == NULL)
  {
    return;
  }
  ...
  (void)HAL_ETH_SetMACFilterConfig(&EthHandle, &filter);
}

static void ethernetif_configure_dma_rx(void)
{
  if (EthHandle.Instance == NULL)
  {
    return;
  ...
  CLEAR_BIT(EthHandle.Instance->DMAOMR, ETH_DMAOMR_DTCEFD);
}
...
static void low_level_init(struct netif *netif)
{
  ...
  ethernetif_derive_mac(s_netif_mac);
  EthHandle.Instance = ETH;
  EthHandle.Init.MACAddr = s_netif_mac;
  ...
  ethernetif_commit_mac_address(s_netif_mac);

  /* set MAC hardware address length */
  netif->hwaddr_len = ETH_HWADDR_LEN;

  /* set MAC hardware address */
  memcpy(netif->hwaddr, s_netif_mac, ETH_HWADDR_LEN);
  ...
  else
  {
    ...
    ethernetif_configure_dma_rx();
    ethernetif_configure_leap_rx();
    netif_set_up(netif);
    netif_set_link_up(netif);
    leap_host_on_link_up(netif);
  }
