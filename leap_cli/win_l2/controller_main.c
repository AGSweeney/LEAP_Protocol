...
int main(int argc, char** argv)
{
    ...
    if (leap_win_handle_version_arg(argc, argv, "leap_win_controller") != 0)
    {
        return 0;
    }

    leap_build_info_print(stdout, "leap_win_controller");
    ...
    stack_config.pd.fixed_digital_outputs    = options.outputs;
    ...
    {
        leap_log_eprintf(
            "bootstrap failed (phase=%u status=%d)\n",
            (unsigned)leap_controller_stack_get_phase(&stack),
            (int)stack.last_status);
        if (stack.last_status == LEAP_CTRL_STACK_UNEXPECTED_REPLY)
        {
            leap_log_eprintf(
                "hint: stale LEAP frame in NIC buffer (retry or wait 1 s); "
                "rebuild controller if this persists\n");
        }
        else if (leap_controller_stack_get_phase(&stack) ==
                 LEAP_CTRL_STACK_SELECT_PROFILE)
        {
            leap_log_eprintf(
                "hint: device may be in SAFE/OP from a prior session; "
                "power-cycle device or wait for lease expiry\n");
        }
