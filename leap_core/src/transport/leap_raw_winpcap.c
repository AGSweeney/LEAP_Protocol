...
typedef void* (*leap_pcap_dump_open_fn)(pcap_t*, const char*);
typedef void (*leap_pcap_dump_fn)(void*, const struct pcap_pkthdr*, const u_char*);
typedef void (*leap_pcap_dump_close_fn)(void*);
...
typedef struct LeapWinpcapApi
{
    ...
    leap_pcap_dump_open_fn         dump_open;
    leap_pcap_dump_fn              dump;
    leap_pcap_dump_close_fn        dump_close;
} LeapWinpcapApi;
...
static void leap_winpcap_copy_friendly_name(
    wchar_t*       wide,
    char*          label,
    size_t         label_cap)
{
...
static int leap_winpcap_gaa_adapter_usable(const IP_ADAPTER_ADDRESSES* cur)
{
    if (cur == NULL || cur->AdapterName == NULL)
    {
        return 0;
    }
    ...
    /* Up or unknown — skip explicitly down/disconnected entries. */
    if (cur->OperStatus == IfOperStatusDown ||
        cur->OperStatus == IfOperStatusNotPresent ||
        cur->OperStatus == IfOperStatusLowerLayerDown)
    {
        return 0;
    }
...
static void leap_winpcap_fill_adapter_from_gaa(
    const IP_ADAPTER_ADDRESSES*  cur,
    LeapRawWinpcapAdapterInfo*   entry)
{
    (void)snprintf(
        entry->path,
        sizeof(entry->path),
        "\\Device\\NPF_%s",
        cur->AdapterName);
    leap_winpcap_copy_friendly_name(
        cur->FriendlyName,
        entry->label,
        sizeof(entry->label));
...
static size_t leap_winpcap_enumerate_adapters_gaa(
    ...
    size_t                     capacity)
{
    ...
    for (cur = addrs; cur != NULL && count < capacity; cur = cur->Next)
    {
        if (leap_winpcap_gaa_adapter_usable(cur) == 0)
        {
            continue;
        }

        leap_winpcap_fill_adapter_from_gaa(cur, &out[count]);
        count++;
    }
...
static size_t leap_winpcap_enumerate_adapters_npcap(
    LeapRawWinpcapAdapterInfo* out,
    size_t                     capacity)
{
    ...
    if (leap_winpcap_load_api() != 0 ||
        g_winpcap.findalldevs == NULL ||
        g_winpcap.freealldevs == NULL)
    {
        return 0u;
    }

    if (g_winpcap.findalldevs(&alldevs, errbuf) != 0 || alldevs == NULL)
    {
        return 0u;
    }

    for (cur = alldevs; cur != NULL && count < capacity; cur = cur->next)
    {
        LeapRawWinpcapAdapterInfo* entry = &out[count];
        ...
        (void)snprintf(entry->path, sizeof(entry->path), "%s", cur->name);
        if (cur->description != NULL && cur->description[0] != '\0')
        {
            (void)snprintf(
                entry->label,
                sizeof(entry->label),
    ...
    size_t                     capacity)
{
    ...
    /*
     * Prefer Npcap's pcap_findalldevs — only adapters Npcap can capture on.
     * The previous GAA+INCLUDE_ALL_INTERFACES path listed every Windows
     * interface (Hyper-V, WSL, TAP, Bluetooth, …) and often hit the 32-slot cap.
     */
    count = leap_winpcap_enumerate_adapters_npcap(out, capacity);
    if (count > 0u)
    {
        return count;
    }

    return leap_winpcap_enumerate_adapters_gaa(out, capacity);
}
...
void leap_raw_winpcap_list_devices(void)
{
    ...
    count = leap_raw_winpcap_enumerate_adapters(
        adapters,
    ...
    for (i = 0u; i < count; i++)
    {
        printf("  %s", adapters[i].path);
        if (adapters[i].label[0] != '\0')
        {
            printf("  (%s)", adapters[i].label);
        }
        if (adapters[i].mac[0] != '\0')
        {
            printf("  %s", adapters[i].mac);
        }
        printf("\n");
    }
    ...
    const LeapRawWinpcapOpenOptions* options)
{
    ...
    if (options != NULL && options->capture_path != NULL &&
        options->capture_path[0] != '\0' &&
        g_winpcap.dump_open != NULL)
    {
        sock->pcap_dump = g_winpcap.dump_open(handle, options->capture_path);
        if (sock->pcap_dump != NULL)
        {
            (void)snprintf(
                sock->capture_path,
                sizeof(sock->capture_path),
                "%s",
...
void leap_raw_winpcap_close(LeapRawWinpcapSocket* sock)
{
    ...
    if (sock->pcap_dump != NULL && g_winpcap.dump_close != NULL)
    {
        g_winpcap.dump_close(sock->pcap_dump);
        sock->pcap_dump = NULL;
    }
...
static void leap_winpcap_maybe_dump(
    LeapRawWinpcapSocket*     sock,
    const struct pcap_pkthdr* header,
    const u_char*             packet)
{
    if (sock != NULL && sock->pcap_dump != NULL && g_winpcap.dump != NULL &&
        header != NULL && packet != NULL)
    {
        g_winpcap.dump(sock->pcap_dump, header, packet);
    }
}
    ...
    *payload_length = 0u;
    ...
    if (timeout_ms == 0)
    {
        for (;;)
        {
            ...
            leap_winpcap_maybe_dump(sock, header, packet);
